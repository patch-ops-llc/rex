...
export default async function SettingsPage() {
  ...
  const calendarConfigured = !!(process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET);
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between">
          <div>
            ...
            <CardDescription>
              Connect your Google Calendar so Rex automatically joins every meeting,
              records transcripts, and extracts action items. Meetings are matched to
              engagements via attendee email addresses.
            </CardDescription>
          </div>
          <AddCalendarButton configured={calendarConfigured} />
        </CardHeader>
        <CardContent>
          {!calendarConfigured ? (
            <div className="rounded-lg border border-dashed p-6 text-center">
              <p className="text-sm text-muted-foreground">
                Set <code className="text-xs bg-muted px-1.5 py-0.5 rounded">GOOGLE_CLIENT_ID</code> and{" "}
                <code className="text-xs bg-muted px-1.5 py-0.5 rounded">GOOGLE_CLIENT_SECRET</code> environment
                variables to enable Google Calendar integration.
              </p>
            </div>
          ) : calendarAccounts.length === 0 ? (
            <p className="text-sm text-muted-foreground py-8 text-center">
              No calendars connected yet. Connect a Google Calendar to start auto-joining meetings.
            </p>
          ) : (
            <div className="space-y-3">
              {calendarAccounts.map((account: any) => (
                ...
                >
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-3 text-xs text-muted-foreground">
                      <span>{account.provider === "GOOGLE" ? "Google Calendar" : account.provider}</span>
                      <span>
                        Added:{" "}
                  ...
                  <CalendarActions accountId={account.id} />
                </div>
              ))}
            </div>
          )}
