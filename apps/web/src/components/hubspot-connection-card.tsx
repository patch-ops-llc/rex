...
export function HubSpotConnectionCard({
  ...
  async function loadPortals() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setAvailablePortals(
          portals.filter((p: PortalInfo) => p.isActive && !linkedIds.has(p.id))
        );
      }
    } catch {
      // silent
  ...
  async function linkPortal(portalId: string) {
    ...
    try {
      const res = await fetch(`/api/engagements/${engagementId}`, {
        ...
        body: JSON.stringify({ addPortalId: portalId }),
      });
      if (res.ok) {
  ...
  async function unlinkPortal(portalId: string) {
    setUnlinkingId(portalId);
    try {
      const res = await fetch(`/api/engagements/${engagementId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ removePortalId: portalId }),
      });
      if (res.ok) {
        router.refresh();
      }
  ...
  async function verifyPortal(portal: PortalInfo) {
    setVerifyingId(portal.id);
    try {
      await fetch(`/api/hubspot-portals/${portal.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  function PortalRow({ portal }: { portal: PortalInfo }) {
    const isVerifying = verifyingId === portal.id;
    const isUnlinking = unlinkingId === portal.id;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <Badge
                variant={portal.isActive ? "success" : "warning"}
                className="text-[10px]"
              >
                {portal.isActive ? "Connected" : "Needs Verification"}
              </Badge>
            </div>
            <p className="text-xs text-muted-foreground">
              Portal ID: {portal.portalId}
              {portal.lastVerifiedAt &&
                ` · Verified ${new Date(portal.lastVerifiedAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}`}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-1">
          {!portal.isActive && (
            <Button
              variant="outline"
              size="sm"
              onClick={() => verifyPortal(portal)}
              disabled={isVerifying}
              className="h-8"
            >
              <RefreshCw
                className={`mr-1.5 h-3 w-3 ${isVerifying ? "animate-spin" : ""}`}
              />
              Verify
            </Button>
          )}
          <Button
            variant="ghost"
            size="sm"
            onClick={() => unlinkPortal(portal.id)}
        ...
        <DialogContent className="sm:max-w-[475px]">
          <form onSubmit={handleNewPortalSubmit}>
            <DialogHeader>
              ...
              <DialogDescription>
                Add a HubSpot portal using a Private App access token. The token
            ...
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setNewPortalForm((f) => ({ ...f, name: e.target.value }))
                  }
                  required
                />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="new-portal-id">Portal ID (Hub ID)</Label>
                <Input
                  id="new-portal-id"
                  placeholder="e.g. 12345678"
                  value={newPortalForm.portalId}
                  onChange={(e) =>
                    setNewPortalForm((f) => ({ ...f, portalId: e.target.value }))
                  }
                  required
                />
              </div>
              <div className="grid gap-2">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Ask the client to create a Private App in HubSpot &rarr;
                  Settings &rarr; Integrations &rarr; Private Apps with CRM
                  read/write scopes. Token is encrypted at rest.
                </p>
              </div>
  ...
  if (linkedPortals.length > 0) {
    const allActive = linkedPortals.every((p) => p.isActive);
